...
if(30<Math.abs(this._initScrollTop-scrollTop)||10<absDScrollTop){if(isScrollingDown&&scrollTop>=maxHeaderTop){top=maxHeaderTop}else if(!isScrollingDown&&scrollTop>=this._dHeight){top=this.condenses&&!this.fixed?this._dHeight:0}var scrollVelocity=dScrollTop/(now-this._lastTimestamp);this.style.transitionDuration=this._clamp((top-lastTop)/scrollVelocity,0,300)+"ms"}else{top=this._top}}}if(0===this._dHeight){progress=0<scrollTop?1:0}else{progress=top/this._dHeight}if(!forceUpdate){this._lastScrollTop=scrollTop;this._top=top;this._wasScrollingDown=isScrollingDown;this._lastTimestamp=now}if(forceUpdate||progress!==this._progress||lastTop!==top||0===scrollTop){this._progress=progress;this._runEffects(progress,top);this._transformHeader(top)}},/**
   ...
   */getScrollState:function(){return{progress:this._progress,top:this._top}}});registerEffect("waterfall",{/** @this {Waterfall} */run:function run(){this.shadow=this.isOnScreen()&&this.isContentBelow()}});/**
    ...
    <slot></slot>
`,is:"app-toolbar"});const PaperSpinnerBehavior={properties:{/**
...
if("production"!==process.env.NODE_ENV){unexpectedKeyCache={}}var shapeAssertionError;try{assertReducerShape(finalReducers)}catch(e){shapeAssertionError=e}return function combination(state,action){if(void 0===state){state={}}if(shapeAssertionError){throw shapeAssertionError}if("production"!==process.env.NODE_ENV){var warningMessage=getUnexpectedStateShapeWarningMessage(state,finalReducers,action,unexpectedKeyCache);if(warningMessage){warning(warningMessage)}}for(var hasChanged=!1,nextState={},_i=0;_i<finalReducerKeys.length;_i++){var _key=finalReducerKeys[_i],reducer=finalReducers[_key],previousStateForKey=state[_key],nextStateForKey=reducer(previousStateForKey,action);if("undefined"===typeof nextStateForKey){var errorMessage=getUndefinedStateErrorMessage(_key,action);throw new Error(errorMessage)}nextState[_key]=nextStateForKey;hasChanged=hasChanged||nextStateForKey!==previousStateForKey}return hasChanged?nextState:state}}function bindActionCreator(actionCreator,dispatch){return function(){return dispatch(actionCreator.apply(this,arguments))}}/**
   ...
   */function isCrushed(){}if("production"!==process.env.NODE_ENV&&"string"===typeof isCrushed.name&&"isCrushed"!==isCrushed.name){warning("You are currently using minified code outside of NODE_ENV === \"production\". "+"This means that you are running a slower development build of Redux. "+"You can use loose-envify (https://github.com/zertosh/loose-envify) for browserify "+"or setting mode to production in webpack (https://webpack.js.org/concepts/mode/) "+"to ensure you have the correct code for your production build.")}var redux={__DO_NOT_USE__ActionTypes:ActionTypes,applyMiddleware:applyMiddleware,bindActionCreators:bindActionCreators,combineReducers:combineReducers,compose:compose,createStore:createStore};_exports.$redux=redux;const UPDATE_PAGE="UPDATE_PAGE";_exports.UPDATE_PAGE=UPDATE_PAGE;const UPDATE_OFFLINE="UPDATE_OFFLINE";_exports.UPDATE_OFFLINE=UPDATE_OFFLINE;const UPDATE_DRAWER_STATE="UPDATE_DRAWER_STATE";_exports.UPDATE_DRAWER_STATE=UPDATE_DRAWER_STATE;const OPEN_SNACKBAR="OPEN_SNACKBAR";_exports.OPEN_SNACKBAR=OPEN_SNACKBAR;const CLOSE_SNACKBAR="CLOSE_SNACKBAR";_exports.CLOSE_SNACKBAR=CLOSE_SNACKBAR;const UPDATE_KEY="UPDATE_KEY";_exports.UPDATE_KEY=UPDATE_KEY;const UPDATE_LANG="UPDATE_LANG";_exports.UPDATE_LANG=UPDATE_LANG;const UPDATE_IS_REG_LOADING="UPDATE_IS_REG_LOADING";_exports.UPDATE_IS_REG_LOADING=UPDATE_IS_REG_LOADING;const navigate=path=>dispatch=>{let key="/"===path?null:path.split("/")[1];if(key){if("register"===key||"overview"===key||"legal"===key){key=null;path="/"}else{path=path.slice(key.length+1)}}// Extract the page name from path.
const page="/"===path||""===path?"overview":path.slice(1);dispatch(updateKey(key));dispatch(loadPage(page));// Close the drawer - in case the *path* change came from a link in the drawer.
dispatch(updateDrawerState(!1))};_exports.navigate=navigate;const loadPage=page=>dispatch=>{switch(page){case"overview":new Promise((res,rej)=>_require.default(["./natepaas-overview.js"],res,rej)).then(bundle=>bundle&&bundle.$natepaasOverview||{}).then(module=>{});break;case"legal":new Promise((res,rej)=>_require.default(["./natepaas-legal.js"],res,rej)).then(bundle=>bundle&&bundle.$natepaasLegal||{});break;default:page="404";new Promise((res,rej)=>_require.default(["./natepaas-404.js"],res,rej)).then(bundle=>bundle&&bundle.$natepaas$404||{});}dispatch(updatePage(page))},updateKey=key=>{return{type:UPDATE_KEY,key}},updatePage=page=>{return{type:UPDATE_PAGE,page}},updateLang=lang=>{return{type:UPDATE_LANG,lang}},updateIsRegLoading=isRegLoading=>{return{type:UPDATE_IS_REG_LOADING,isRegLoading}};_exports.updateIsRegLoading=updateIsRegLoading;let snackbarTimer;const showSnackbar=snackbarText=>dispatch=>{dispatch({type:OPEN_SNACKBAR,snackbarText:snackbarText});window.clearTimeout(snackbarTimer);snackbarTimer=window.setTimeout(()=>dispatch({type:CLOSE_SNACKBAR}),6e3)};_exports.showSnackbar=showSnackbar;const updateOffline=offline=>(dispatch,getState)=>{// Show the snackbar only if offline status changes.
if(offline!==getState().app.offline){const snackbarText="You are now "+offline?"offline":"online";dispatch(showSnackbar(snackbarText))}dispatch({type:UPDATE_OFFLINE,offline})};_exports.updateOffline=updateOffline;const updateDrawerState=opened=>{return{type:UPDATE_DRAWER_STATE,opened}};_exports.updateDrawerState=updateDrawerState;var app={UPDATE_PAGE:UPDATE_PAGE,UPDATE_OFFLINE:UPDATE_OFFLINE,UPDATE_DRAWER_STATE:UPDATE_DRAWER_STATE,OPEN_SNACKBAR:OPEN_SNACKBAR,CLOSE_SNACKBAR:CLOSE_SNACKBAR,UPDATE_KEY:UPDATE_KEY,UPDATE_LANG:UPDATE_LANG,UPDATE_IS_REG_LOADING:UPDATE_IS_REG_LOADING,navigate:navigate,updateIsRegLoading:updateIsRegLoading,showSnackbar:showSnackbar,updateOffline:updateOffline,updateDrawerState:updateDrawerState};_exports.$app=app;const GET_ASSETS="GET_ASSETS";_exports.GET_ASSETS=GET_ASSETS;const loadAssets=key=>dispatch=>{const url="https://europe-west1-natepaas.cloudfunctions.net/assets";if(key){fetch(url,{method:"POST",mode:"cors",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:key})}).then(response=>response.json()).then(assets=>{dispatch({type:GET_ASSETS,assets})}).catch(error=>{console.error(error)})}};_exports.loadAssets=loadAssets;var assets={GET_ASSETS:GET_ASSETS,loadAssets:loadAssets};_exports.$assets=assets;const menuIcon=html$1`<svg height="24" viewBox="0 0 24 24" width="24"><path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"></path></svg>`;_exports.menuIcon=menuIcon;var myIcons={menuIcon:menuIcon};_exports.$myIcons=myIcons;const INITIAL_STATE={page:"",key:null,lang:"de",offline:!1,drawerOpened:!1,snackbarOpened:!1},app$1=(state=INITIAL_STATE,action)=>{switch(action.type){case UPDATE_PAGE:return _objectSpread({},state,{page:action.page});case UPDATE_KEY:return _objectSpread({},state,{key:action.key});case UPDATE_LANG:return _objectSpread({},state,{lang:action.lang});case UPDATE_OFFLINE:return _objectSpread({},state,{offline:action.offline});case UPDATE_DRAWER_STATE:return _objectSpread({},state,{drawerOpened:action.opened});case OPEN_SNACKBAR:return _objectSpread({},state,{snackbarOpened:!0,snackbarText:action.snackbarText});case CLOSE_SNACKBAR:return _objectSpread({},state,{snackbarOpened:!1});case UPDATE_IS_REG_LOADING:return _objectSpread({},state,{isRegLoading:action.isRegLoading});default:return state;}};_exports.$appDefault=app$1;var app$2={default:app$1};_exports.$app$1=app$2;const devCompose=window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__||compose,store=createStore(state=>state,devCompose(lazyReducerEnhancer(combineReducers),applyMiddleware(thunk)));_exports.store=store;store.addReducers({app:app$1});var store$1={store:store};_exports.$store=store$1;const INITIAL_STATE$1={images:[],texts:[]},assets$1=(state=INITIAL_STATE$1,action)=>{switch(action.type){case GET_ASSETS:return{images:action.assets.images,texts:action.assets.texts};default:return state;}};_exports.$assetsDefault=assets$1;var assets$2={default:assets$1};_exports.$assets$1=assets$2;class SnackBar extends LitElement{static get properties(){return{active:{type:Boolean}}}static get styles(){return[css`
        :host {
          display: block;
          position: fixed;
          top: 100%;
      ...
      <slot></slot>
    `}}window.customElements.define("snack-bar",SnackBar);store.addReducers({assets:assets$1});class NatePaasApp extends connect(store)(LitElement){static get properties(){return{appTitle:{type:String},_page:{type:String},_drawerOpened:{type:Boolean},_snackbarOpened:{type:Boolean},_snackbarText:{type:String},_offline:{type:Boolean},_key:{type:String}}}static get styles(){return[css`
        :host {
          display: block;
        ...
        //   }
        // }
      `]}render(){const page=this._page,key=this._key,ovw_path=key?"/"+key+"/overview":"/overview",legal_path=key?"/"+key+"/legal":"/legal",reg_path=key?"/"+key+"/register":"/register";return html$1`
      <app-header condenses reveals effects="waterfall">
        ...
        <nav class="toolbar-list">
          <a ?selected="${"overview"===page}" href="${ovw_path}">Übersicht</a>
          <a ?selected="${"legal"===page}" href="${legal_path}">Impressum</a>
        </nav>
      </app-header>
