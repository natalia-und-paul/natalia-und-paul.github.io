...
</dom-module>`;document.head.appendChild($_documentContainer$1.content);const template$5=html`
...
if("production"!==process.env.NODE_ENV){unexpectedKeyCache={}}var shapeAssertionError;try{assertReducerShape(finalReducers)}catch(e){shapeAssertionError=e}return function combination(state,action){if(void 0===state){state={}}if(shapeAssertionError){throw shapeAssertionError}if("production"!==process.env.NODE_ENV){var warningMessage=getUnexpectedStateShapeWarningMessage(state,finalReducers,action,unexpectedKeyCache);if(warningMessage){warning(warningMessage)}}for(var hasChanged=!1,nextState={},_i=0;_i<finalReducerKeys.length;_i++){var _key=finalReducerKeys[_i],reducer=finalReducers[_key],previousStateForKey=state[_key],nextStateForKey=reducer(previousStateForKey,action);if("undefined"===typeof nextStateForKey){var errorMessage=getUndefinedStateErrorMessage(_key,action);throw new Error(errorMessage)}nextState[_key]=nextStateForKey;hasChanged=hasChanged||nextStateForKey!==previousStateForKey}return hasChanged?nextState:state}}function bindActionCreator(actionCreator,dispatch){return function(){return dispatch(actionCreator.apply(this,arguments))}}/**
   ...
   */function isCrushed(){}if("production"!==process.env.NODE_ENV&&"string"===typeof isCrushed.name&&"isCrushed"!==isCrushed.name){warning("You are currently using minified code outside of NODE_ENV === \"production\". "+"This means that you are running a slower development build of Redux. "+"You can use loose-envify (https://github.com/zertosh/loose-envify) for browserify "+"or setting mode to production in webpack (https://webpack.js.org/concepts/mode/) "+"to ensure you have the correct code for your production build.")}var redux={__DO_NOT_USE__ActionTypes:ActionTypes,applyMiddleware:applyMiddleware,bindActionCreators:bindActionCreators,combineReducers:combineReducers,compose:compose,createStore:createStore};const UPDATE_PAGE="UPDATE_PAGE",UPDATE_OFFLINE="UPDATE_OFFLINE",UPDATE_DRAWER_STATE="UPDATE_DRAWER_STATE",OPEN_SNACKBAR="OPEN_SNACKBAR",CLOSE_SNACKBAR="CLOSE_SNACKBAR",UPDATE_KEY="UPDATE_KEY",UPDATE_LANG="UPDATE_LANG",UPDATE_IS_REG_LOADING="UPDATE_IS_REG_LOADING",OPEN_DIALOG="OPEN_DIALOG",CLOSE_DIALOG="CLOSE_DIALOG",UPDATE_ATTENDANCE="UPDATE_ATTENDANCE",navigate=(path,query)=>dispatch=>{let key=null;query.slice(1).split("&").forEach(kv=>{const k=kv.split("=")[0],v=kv.split("=")[1];if("k"==k){key=v}});const page="/"===path?"overview":path.slice(1);if(key){dispatch(updateKey(key))}dispatch(loadPage(page));dispatch(updateDrawerState(!1))},loadPage=page=>dispatch=>{switch(page){case"overview":import("./natepaas-overview.js").then(bundle=>bundle&&bundle.$natepaasOverview||{}).then(module=>{});break;case"legal":import("./natepaas-legal.js").then(bundle=>bundle&&bundle.$natepaasLegal||{});break;default:page="404";import("./natepaas-404.js").then(bundle=>bundle&&bundle.$natepaas$404||{});}dispatch(updatePage(page))},updateKey=key=>{return{type:UPDATE_KEY,key}},updatePage=page=>{return{type:UPDATE_PAGE,page}},updateLang=lang=>{return{type:UPDATE_LANG,lang}},updateIsRegLoading=isRegLoading=>{return{type:UPDATE_IS_REG_LOADING,isRegLoading}};let snackbarTimer;const showSnackbar=snackbarText=>dispatch=>{dispatch({type:OPEN_SNACKBAR,snackbarText:snackbarText});window.clearTimeout(snackbarTimer);snackbarTimer=window.setTimeout(()=>dispatch({type:CLOSE_SNACKBAR}),6e3)},openDialog=dialogText=>dispatch=>{dispatch({type:OPEN_DIALOG,dialogText:dialogText})},closeDialog=()=>dispatch=>{dispatch({type:CLOSE_DIALOG})},updateOffline=offline=>(dispatch,getState)=>{// Show the snackbar only if offline status changes.
if(offline!==getState().app.offline){const snackbarText="You are now "+offline?"offline":"online";dispatch(showSnackbar(snackbarText))}dispatch({type:UPDATE_OFFLINE,offline})},updateDrawerState=opened=>{return{type:UPDATE_DRAWER_STATE,opened}},updateAttendance=isAttending=>{return{type:UPDATE_ATTENDANCE,isAttending}};var app={UPDATE_PAGE:UPDATE_PAGE,UPDATE_OFFLINE:UPDATE_OFFLINE,UPDATE_DRAWER_STATE:UPDATE_DRAWER_STATE,OPEN_SNACKBAR:OPEN_SNACKBAR,CLOSE_SNACKBAR:CLOSE_SNACKBAR,UPDATE_KEY:UPDATE_KEY,UPDATE_LANG:UPDATE_LANG,UPDATE_IS_REG_LOADING:UPDATE_IS_REG_LOADING,OPEN_DIALOG:OPEN_DIALOG,CLOSE_DIALOG:CLOSE_DIALOG,UPDATE_ATTENDANCE:UPDATE_ATTENDANCE,navigate:navigate,updateLang:updateLang,updateIsRegLoading:updateIsRegLoading,showSnackbar:showSnackbar,openDialog:openDialog,closeDialog:closeDialog,updateOffline:updateOffline,updateDrawerState:updateDrawerState,updateAttendance:updateAttendance};const GET_ASSETS="GET_ASSETS",loadAssets=key=>dispatch=>{const url="https://europe-west1-natepaas.cloudfunctions.net/assets-2";if(key){fetch(url,{method:"POST",mode:"cors",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:key})}).then(response=>response.json()).then(assets=>{dispatch({type:GET_ASSETS,assets})}).catch(error=>{console.error(error)})}};var assets={GET_ASSETS:GET_ASSETS,loadAssets:loadAssets};const menuIcon=html$1`<svg height="24" viewBox="0 0 24 24" width="24"><path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"></path></svg>`,translateIcon=html$1`<svg height="24" viewBox="0 0 24 24" width="24"><path d="M12.87,15.07L10.33,12.56L10.36,12.53C12.1,10.59 13.34,8.36 14.07,6H17V4H10V2H8V4H1V6H12.17C11.5,7.92 10.44,9.75 9,11.35C8.07,10.32 7.3,9.19 6.69,8H4.69C5.42,9.63 6.42,11.17 7.67,12.56L2.58,17.58L4,19L9,14L12.11,17.11L12.87,15.07M18.5,10H16.5L12,22H14L15.12,19H19.87L21,22H23L18.5,10M15.88,17L17.5,12.67L19.12,17H15.88Z"></path></svg>`,starIcon=html$1`<svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"/><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/><path d="M0 0h24v24H0z" fill="none"/></svg>`;var myIcons={menuIcon:menuIcon,translateIcon:translateIcon,starIcon:starIcon};const getLanguage=()=>{if("de"===navigator.language.split("-")[0]){return"de"}else if("en"===navigator.language.split("-")[0]){return"en"}else if("ru"===navigator.language.split("-")[0]){return"ru"}else{return"de";// The default of this page
}},INITIAL_STATE={page:"",key:null,lang:"de",// getLanguage(),
offline:!1,drawerOpened:!1,snackbarOpened:!1,isAttending:!0},app$1=(state=INITIAL_STATE,action)=>{switch(action.type){case UPDATE_PAGE:return{...state,page:action.page};case UPDATE_KEY:return{...state,key:action.key};case UPDATE_LANG:return{...state,lang:action.lang};case UPDATE_OFFLINE:return{...state,offline:action.offline};case UPDATE_DRAWER_STATE:return{...state,drawerOpened:action.opened};case OPEN_SNACKBAR:return{...state,snackbarOpened:!0,snackbarText:action.snackbarText};case CLOSE_SNACKBAR:return{...state,snackbarOpened:!1};case OPEN_DIALOG:return{...state,openDialog:!0,dialogText:action.dialogText};case CLOSE_DIALOG:return{...state,openDialog:!1};case UPDATE_IS_REG_LOADING:return{...state,isRegLoading:action.isRegLoading};case UPDATE_ATTENDANCE:return{...state,isAttending:action.isAttending};default:return state;}};var app$2={default:app$1};const devCompose=window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__||compose,store=createStore(state=>state,devCompose(lazyReducerEnhancer(combineReducers),applyMiddleware(thunk)));store.addReducers({app:app$1});var store$1={store:store};const INITIAL_STATE$1={images:[],texts:[]},assets$1=(state=INITIAL_STATE$1,action)=>{switch(action.type){case GET_ASSETS:return{images:action.assets.images,texts:action.assets.texts};default:return state;}};var assets$2={default:assets$1};class SnackBar extends LitElement{static get properties(){return{active:{type:Boolean}}}static get styles(){return[css`
        :host {
          display: block;
          position: fixed;
          bottom: 100%;
      ...
      <slot></slot>
    `}}window.customElements.define("snack-bar",SnackBar);store.addReducers({assets:assets$1});class NatePaasApp extends connect(store)(LitElement){static get properties(){return{appTitle:{type:String},_page:{type:String},_lang:{type:String},_drawerOpened:{type:Boolean},_snackbarOpened:{type:Boolean},_snackbarText:{type:String},_offline:{type:Boolean},_dialogOpened:{type:Boolean},_dialogText:{type:String},_key:{type:String}}}static get styles(){return[css`
        :host {
          display: block;
        ...
        //   }
        // }
      `]}render(){const page=this._page,lang=this._lang,overviewPath="/overview"+(this._key?"?k="+this._key:""),legalPath="/legal"+(this._key?"?k="+this._key:"");let overviewTitle,legalTitle,langTitle;if("de"===lang){overviewTitle="\xDCbersicht";legalTitle="Impressum";langTitle="Sprache"}else if("en"===lang){overviewTitle="Overview";legalTitle="Legal";langTitle="Language"}else if("ru"===lang){overviewTitle="\u041E\u0431\u0437\u043E\u0440";legalTitle="\u041A\u043E\u043D\u0444\u0438\u0434\u0435\u043D\u0446\u0438\u0430\u043B\u044C\u043D\u043E\u0441\u0442\u044C";langTitle="\u042F\u0437\u044B\u043A"}else{overviewTitle="\xDCbersicht";legalTitle="Impressum";langTitle="Sprache"}return html$1`
      <app-header condenses reveals effects="waterfall">
        ...
        <nav class="toolbar-list">
          <a ?selected="${"overview"===page}" href="${overviewPath}">${overviewTitle}</a>
          <a ?selected="${"legal"===page}" href="${legalPath}">${legalTitle}</a>
        </nav>
      </app-header>
      ...
      <app-drawer
        ...
        <nav class="drawer-list">
          ...
          <div class="drawer-subtitle">
            ${translateIcon} ${langTitle}
          </div>
          <a
            class="lang"
            ?selected="${"de"===lang}"
            @click="${this._changeLangDe}">Deutsch</a>
          <a
            class="lang"
            ?selected="${"en"===lang}"
            @click="${this._changeLangEn}">English</a>
          <a
            class="lang"
            ?selected="${"ru"===lang}"
            @click="${this._changeLangRu}">русский</a>
        </nav>
      </app-drawer>

      <main role="main" class="main-content">
        <natepaas-overview class="page" ?active="${"overview"===this._page}"></natepaas-overview>
        <natepaas-legal class="page" ?active="${"legal"===this._page}"></natepaas-legal>
        <natepaas-404 class="page" ?active="${"404"===this._page}"></natepaas-404>
      </main>
    ...
    `}constructor(){super();setPassiveTouchGestures(!0)}firstUpdated(){installRouter(location=>store.dispatch(navigate(decodeURIComponent(location.pathname),decodeURIComponent(location.search))));installOfflineWatcher(offline=>store.dispatch(updateOffline(offline)));installMediaQueryWatcher(`(min-width: 460px)`,()=>store.dispatch(updateDrawerState(!1)))}updated(changedProps){if(changedProps.has("_page")){const pageTitle=this.appTitle+" - "+this._page;updateMetadata({title:pageTitle,description:pageTitle})}}_menuButtonClicked(){store.dispatch(updateDrawerState(!0))}_drawerOpenedChanged(e){store.dispatch(updateDrawerState(e.target.opened))}_dialogClosed(){store.dispatch(closeDialog())}_changeLangEn(){store.dispatch(updateLang("en"));store.dispatch(updateDrawerState(!1))}_changeLangDe(){store.dispatch(updateLang("de"));store.dispatch(updateDrawerState(!1))}_changeLangRu(){store.dispatch(updateLang("ru"));store.dispatch(updateDrawerState(!1))}stateChanged(state){this._page=state.app.page;this._offline=state.app.offline;this._snackbarOpened=state.app.snackbarOpened;this._drawerOpened=state.app.drawerOpened;this._snackbarText=state.app.snackbarText;this._dialogText=state.app.dialogText;this._lang=state.app.lang;if(this._key!==state.app.key){this._key=state.app.key;store.dispatch(loadAssets(this._key))}if(this._dialogOpened!==state.app.openDialog){if(state.app.openDialog){this.shadowRoot.getElementById("modal").open();this._dialogOpened=!0}else{this.shadowRoot.getElementById("modal").close();this._dialogOpened=!1}}}}window.customElements.define("natepaas-app",NatePaasApp);class PageViewElement extends LitElement{shouldUpdate(){return this.active}static get properties(){return{active:{type:Boolean}}}}var pageViewElement={PageViewElement:PageViewElement};const SharedStyles=css`
  :host {
    display: block;
    box-sizing: border-box;
  }
